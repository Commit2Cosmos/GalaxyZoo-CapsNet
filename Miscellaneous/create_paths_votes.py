...
# merged = pd.read_csv('./Data/training_solutions_rev1.csv', usecols=[0,1,2])
merged = pd.read_csv('./InitData/Kaggle/labels.csv', usecols=[0,1,2])

merged['GalaxyID'] = merged.iloc[:,0].map(lambda x:f'{x}.jpg')

filenames = os.listdir('./InitData/Kaggle/images_kaggle')
...
# print(merged)
merged.to_csv('./PreparedData/Kaggle/RGB/paths_votes.csv', encoding='utf-8', index=False)